...
class StaticPage extends Controller
{
    ...
    private function mark(){ // 随机产生10条mark信息
        $marks = [];
        $length = Mark::count();
    ...
    private function makeChart($max, $modelClass){ // 用户注册统计表
        $chart = [];
        for($i=0; $i < $max; $i++){
    ...
    public function getTest(){
        $data['end_time'] = Carbon::now()->subMonth(0)->endOfMonth();
        $data['start_time'] = Carbon::now()->subMonth(1)->endOfMonth();
        $data['user'] = User::where('created_at', '<', $data['end_time'])
